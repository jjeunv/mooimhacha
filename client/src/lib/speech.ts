...
function addPunctuation(text: string): string {
  return text
    ...
    .replace(/(해요|있어요|없어요|같아요|예요|이에요|네요|거예요|래요|대요|아요|어요|하고요|이고요|이에요|감사합니다|안녕하세요)([ ]+)/g, '$1. ')
    // 의문형
    .replace(/(습니까|합니까|인가요|나요|을까요|ㄹ까요|인지요)([ ]+)/g, '$1? ')
    // 마지막 문장 마침표 (아직 없으면)
    .replace(/(습니다|있습니다|없습니다|같습니다|됩니다|입니다|겠습니다|드립니다|해요|있어요|없어요|같아요|예요|이에요|네요|거예요|하고요|이고요|아요|어요|감사합니다|안녕하세요)$/, '$1.');
}
